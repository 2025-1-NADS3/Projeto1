...
public class RegisterActivity extends AppCompatActivity {
    ...
    private void registrarUsuario() {
        ...
        JsonObjectRequest request = new JsonObjectRequest(Request.Method.POST, url, dadosUsuario,
                ...
                error -> {
                    try {
                        // Realiza uma verficação se tem resposta de erro do backend
                        if (error.networkResponse != null && error.networkResponse.data != null) {
                            String respostaBody = new String(error.networkResponse.data, "utf-8");
                            JSONObject jsonError = new JSONObject(respostaBody);

                            // Captura a mensagem de erro do backend e exibe ao usuário
                            String mensagemErro = jsonError.optString("erro", "Erro desconhecido ao cadastrar.");
                            Toast.makeText(RegisterActivity.this, mensagemErro, Toast.LENGTH_LONG).show();
                        } else {
                            Toast.makeText(RegisterActivity.this, "Erro na conexão com o servidor.", Toast.LENGTH_LONG).show();
                        }
                    } catch (Exception e) {
                        e.printStackTrace();
                        Toast.makeText(RegisterActivity.this, "Erro inesperado ao processar resposta.", Toast.LENGTH_LONG).show();
                    }
                });
